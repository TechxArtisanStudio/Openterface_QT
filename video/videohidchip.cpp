...
QPair<QByteArray, bool> Ms2109Chip::read4Byte(quint16 address)
{
    ...
    qCDebug(log_chip_read).nospace() << "MS2109 reading from address: 0x"
    ...
    if (m_transport->sendFeatureReport(reinterpret_cast<uint8_t*>(ctrlData.data()), ctrlData.size())) {
        if (m_transport->getFeatureReport(reinterpret_cast<uint8_t*>(result.data()), result.size())) {
            ...
            qCDebug(log_chip_read).nospace() << "MS2109 read OK addr=0x"
                << QString::number(address, 16).rightJustified(4, '0').toUpper()
        ...
        if (m_transport->sendFeatureReport(reinterpret_cast<uint8_t*>(ctrlData.data()), ctrlData.size())) {
            if (m_transport->getFeatureReport(reinterpret_cast<uint8_t*>(result.data()), result.size()) && !result.isEmpty()) {
                ...
                qCDebug(log_chip_read) << "MS2109 read OK (alt) addr=0x" << QString::number(address, 16);
                return qMakePair(readResult, true);
            }
        }
    }
    qCWarning(log_chip_read) << "MS2109 read FAILED addr=0x" << QString::number(address, 16);
...
QPair<QByteArray, bool> Ms2109sChip::read4Byte(quint16 address)
{
    ...
    if (!wasOpen) {
        if (!m_transport->open()) {
            qCWarning(log_chip_read) << "MS2109S read: failed to open transport";
            return qMakePair(QByteArray(4, 0), false);
        }
    }
    ...
    if (!success) {
        qCWarning(log_chip_read) << "MS2109S all read attempts failed addr=0x" << QString::number(address, 16);
        return qMakePair(QByteArray(4, 0), false);
    }
...
QPair<QByteArray, bool> Ms2130sChip::read4Byte(quint16 address)
{
    ...
    if (!wasOpen) {
        if (!m_transport->open()) {
            qCWarning(log_chip_read) << "MS2130S read: failed to open transport";
            return qMakePair(QByteArray(4, 0), false);
        }
    }
    ...
    qCDebug(log_chip_read) << "MS2130S reading addr=0x" << QString::number(address, 16).rightJustified(4, '0').toUpper();
    ...
    if (!success) {
        qCWarning(log_chip_read) << "MS2130S all read attempts failed addr=0x" << QString::number(address, 16);
        return qMakePair(QByteArray(4, 0), false);
    }
    QByteArray finalResult(4, 0);
    finalResult[0] = readResult[0];
...
int Ms2130sChip::detectConnectMode()
{
    ...
    if (m_transport->sendDirect(setData, 9)) {
        ...
        if (m_transport->sendDirect(setData, 9)) {
            ...
            if (m_transport->sendDirect(setData, 9)) {
                ...
                if (m_transport->getDirect(getData, 9)) {
                    ...
                    qCInfo(log_chip_flash) << "MS2130S connect mode V1 probe recv =" << hexRecv.trimmed();
                    if (getData[4] == 0x13) {
                        qCInfo(log_chip_flash) << "MS2130S connect mode: V1";
                        return 1;
                    }
                }
            }
        ...
        qCDebug(log_chip_flash) << "MS2130S V1 probe sendDirect failed, falling back to V2/V3";
    }
    ...
    if (!m_transport->sendDirect(setData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S connect mode: V2/V3 sendDirect failed";
        return 0;
    }
    getData[0] = 0x01;
    if (!m_transport->getDirect(getData, 9)) {
    ...
    qCInfo(log_chip_flash) << "MS2130S connect mode V2/V3 probe sent =" << hexSent.trimmed();
    qCInfo(log_chip_flash) << "MS2130S connect mode V2/V3 probe recv =" << hexRecv.trimmed();
    for (int i = 0; i < 4; ++i) {
        if (setData[i] != getData[i]) {
            qCInfo(log_chip_flash) << "MS2130S connect mode: V2 (byte" << i << "differs)";
            return 2;
        }
    }
    qCInfo(log_chip_flash) << "MS2130S connect mode: V3 (bytes 0-3 match)";
    return 3;
}
...
bool Ms2130sChip::initializeGPIO()
{
    ...
    qCDebug(log_chip_gpio) << "MS2130S initializing GPIO for flash operations...";

    if (connectMode == 0) {
        connectMode = detectConnectMode();
        if (connectMode == 0) {
            qCWarning(log_chip_flash) << "MS2130S connect mode detection failed";
            return false;
        }
    }
    ...
    auto write8 = [&](quint8 addr, quint8 value) -> bool {
        ...
        if (!ok) qCWarning(log_chip_gpio) << "MS2130S GPIO write8 failed: addr="
    ...
    auto read8 = [&](quint8 addr, quint8 &value) -> bool {
        ...
        if (!m_transport->sendDirect(s, 9)) {
            qCWarning(log_chip_gpio) << "MS2130S GPIO read8 send failed: addr="
                << QString("0x%1").arg(addr, 2, 16, QChar('0'));
            return false;
        }
        uint8_t g[9] = {}; g[0] = reportId;
        if (!m_transport->getDirect(g, 9)) {
            qCWarning(log_chip_gpio) << "MS2130S GPIO read8 get failed: addr="
                << QString("0x%1").arg(addr, 2, 16, QChar('0'));
            return false;
        }
    ...
    auto write16 = [&](quint16 addr, quint8 value) -> bool {
        ...
        bool ok = m_transport->sendDirect(buf, 9);
        if (!ok) qCWarning(log_chip_gpio) << "MS2130S GPIO write16 failed: addr="
    ...
    auto read16 = [&](quint16 addr, quint8 &value) -> bool {
        ...
        if (!m_transport->sendDirect(s, 9)) {
            qCWarning(log_chip_gpio) << "MS2130S GPIO read16 send failed: addr="
                << QString("0x%1").arg(addr, 4, 16, QChar('0'));
            return false;
        }
        uint8_t g[9] = {}; g[0] = reportId;
        if (!m_transport->getDirect(g, 9)) {
            qCWarning(log_chip_gpio) << "MS2130S GPIO read16 get failed: addr="
                << QString("0x%1").arg(addr, 4, 16, QChar('0'));
            return false;
        }
    ...
    if (!read8(0xB0, gpio_saved_b0))  { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed read 0xB0"; return false; }
    if (!write8(0xB0, gpio_saved_b0 & static_cast<quint8>(~0x04))) { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed write 0xB0"; return false; }
    if (!read8(0xA0, gpio_saved_a0))  { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed read 0xA0"; return false; }
    if (!write8(0xA0, gpio_saved_a0 | 0x04)) { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed write 0xA0"; return false; }
    read8(0xC7, gpio_saved_c7);
    if (!write8(0xC7, 0xD1)) { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed write 0xC7"; return false; }
    read8(0xC8, gpio_saved_c8);
    if (!write8(0xC8, 0xC0)) { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed write 0xC8"; return false; }
    read8(0xCA, gpio_saved_ca);
    if (!write8(0xCA, 0x00)) { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed write 0xCA"; return false; }
    if (!read16(0xF01F, gpio_saved_f01f)) { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed read 0xF01F"; return false; }
    quint8 f01fMod = (gpio_saved_f01f | 0x10) & static_cast<quint8>(~0x80);
    if (!write16(0xF01F, f01fMod)) { qCWarning(log_chip_gpio) << "MS2130S GPIO init: failed write 0xF01F"; return false; }

    gpioSaved = true;
    qCInfo(log_chip_gpio) << "MS2130S GPIO initialization completed successfully";
    return true;
}
...
{
    if (!gpioSaved) {
        qCDebug(log_chip_gpio) << "MS2130S GPIO restore: nothing saved, skipping";
        return;
    }
    if (!m_transport) return;

    qCInfo(log_chip_gpio) << "MS2130S restoring GPIO registers to pre-flash values...";
    const int    mode     = connectMode;
    const quint8 reportId = (mode == 1) ? 0x00 : 0x01;
    ...
    qCInfo(log_chip_gpio) << "MS2130S GPIO registers restored";
    gpioSaved = false;
}
...
bool Ms2130sChip::eraseSector(quint32 startAddress)
{
    ...
    if (!m_transport->sendDirect(ctrlData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S sector erase failed at"
            << QString("0x%1").arg(startAddress, 8, 16, QChar('0'));
        return false;
    }
    qCDebug(log_chip_flash) << "MS2130S sector erase complete at"
        << QString("0x%1").arg(startAddress, 8, 16, QChar('0'));
    return true;
}
...
bool Ms2130sChip::flashEraseDone(bool &done)
{
    ...
    if (!openedForOp) {
        qCWarning(log_chip_flash) << "MS2130S erase-done: failed to open device handle";
        done = false;
        return true;
    }
    uint8_t ctrlData[9] = {0};
    ...
    if (!m_transport->sendDirect(ctrlData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S erase-done sendDirect failed";
    ...
    if (!m_transport->getDirect(getData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S erase-done getDirect failed";
        if (!m_transport->isOpen()) m_transport->close();
        done = false;
        return true;
    }
    ...
    qCDebug(log_chip_flash) << "MS2130S erase-done status byte[2]:"
...
bool Ms2130sChip::flashGetSize(quint32 &flashSize)
{
    ...
    if (!m_transport->sendDirect(ctrlData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S flashGetSize: SetFeature 0xF4 failed";
        return false;
    }

    uint8_t getData[9] = {0x01, 0, 0, 0, 0, 0, 0, 0, 0};
    if (!m_transport->getDirect(getData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S flashGetSize: GetFeature 0xF4 failed";
    ...
    qCInfo(log_chip_flash) << "MS2130S flashGetSize: flash size =" << flashSize
...
bool Ms2130sChip::flashBurstWrite(quint32 address, const QByteArray &data)
{
    ...
    if (connectMode == 1) {
        ...
        if (!m_transport->sendDirect(ctrlData, 9)) {
            qCWarning(log_chip_flash) << "MS2130S V1 burst write init failed";
            return false;
        }
        for (quint16 offset = 0; offset < u16_length; offset = static_cast<quint16>(offset + 6)) {
            ...
            if (!m_transport->sendDirect(packet, 9)) {
                qCWarning(log_chip_flash) << "MS2130S V1 burst write data packet failed at offset" << offset;
                return false;
            }
    ...
    ctrlData[7] = static_cast<uint8_t>(u16_length & 0xFF);
    {
        ...
        qCInfo(log_chip_flash) << "MS2130S burst write 0xE7 init packet:" << hexDump.trimmed()
    ...
    if (!m_transport->sendDirect(ctrlData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S burst write 0xE7 init failed";
        return false;
    }
    ...
    for (j = 0; j < cnt; ++j) {
        ...
        if (j == 0) {
            ...
            qCInfo(log_chip_flash) << "MS2130S burst write first 0x03 packet (17 bytes):" << hexDump.trimmed();
        }
        if (!m_transport->sendDirect(reinterpret_cast<uint8_t*>(buffer.data()), u16_bufferSize + 1)) {
            qCWarning(log_chip_flash) << "MS2130S burst write data chunk" << j << "failed";
            return false;
        }
    ...
    if (left != 0) {
        ...
        if (!m_transport->sendDirect(reinterpret_cast<uint8_t*>(buffer.data()), u16_bufferSize + 1)) {
            qCWarning(log_chip_flash) << "MS2130S burst write final chunk failed";
            return false;
        }
        written_size += left;
        if (onChunkWritten) onChunkWritten(written_size);
...
bool Ms2130sChip::flashBurstRead(quint32 address, quint32 length, QByteArray &outData)
{
    ...
    ctrlData[7] = static_cast<uint8_t>(u16_length & 0xFF);
    if (!m_transport->sendDirect(ctrlData, 9)) {
        qCWarning(log_chip_flash) << "MS2130S burst read 0xE7 init failed";
        return false;
    }
    ...
    for (int j = 0; j < cnt; ++j) {
        if (!getPacket()) {
            qCWarning(log_chip_flash) << "MS2130S burst read chunk" << j << "/" << cnt << "failed";
            return false;
        }
        if (j == 0) {
            QString hexDump;
            for (int k = 0; k < 17; ++k) hexDump += QString("%1 ").arg(static_cast<uint8_t>(buffer[k]), 2, 16, QChar('0'));
            qCInfo(log_chip_flash) << "MS2130S burst read first packet (17 bytes):" << hexDump.trimmed()
    ...
    if (left != 0) {
        if (!getPacket()) {
            qCWarning(log_chip_flash) << "MS2130S burst read final chunk failed";
            return false;
        }
        {
            QString hexDump;
            ...
            qCInfo(log_chip_flash) << "MS2130S burst read final chunk (first bytes):" << hexDump.trimmed()
                             << " (buf[0]=" << QString("0x%1").arg(static_cast<uint8_t>(buffer[0]), 2, 16, QChar('0')) << ")";
        }
        outData.append(buffer.constData() + 1, left);
    }
    qCDebug(log_chip_flash) << "MS2130S burst read complete:" << outData.size() << "bytes from"
...
bool Ms2130sChip::writeFirmware(quint16 address, const QByteArray &data)
{
    ...
    if (!m_transport->open()) {
        qCWarning(log_chip_flash) << "MS2130S could not begin transaction for firmware write";
        return false;
    }
    ...
    qCInfo(log_chip_gpio) << "MS2130S flash flow: overlapped handle (matches reference MsHidLink), GPIO restore disabled, soft-reset if verify passes";

    bool ok = true;
    bool gpioInitialized = false;

    if (!initializeGPIO()) {
        qCWarning(log_chip_gpio) << "MS2130S GPIO initialization failed – aborting firmware write";
        ok = false;
    } else {
        gpioInitialized = true;
        qCInfo(log_chip_flash) << "MS2130S detected connect mode for flash:" << connectMode;
    }

    if (ok && connectMode == 1) {
        qCWarning(log_chip_flash) << "MS2130S V1 mode is not supported by current updater flow. Aborting.";
        ok = false;
    }
    ...
    if (ok) {
        ...
        if (!flashGetSize(detectedFlashSize)) {
            qCWarning(log_chip_flash) << "MS2130S flashGetSize (0xF4 handshake) failed – write may not succeed";
            ...
            qCInfo(log_chip_flash) << "MS2130S entered Flash-programming mode; detected flash size:"
    ...
    if (ok) {
        ...
        qCDebug(log_chip_flash) << "MS2130S erasing" << numSectors << "sector(s)";
        ...
        for (int i = 0; i < numSectors; ++i) {
            ...
            if (!eraseSector(sectorAddr)) {
                qCWarning(log_chip_flash) << "MS2130S sector erase failed at" << QString("0x%1").arg(sectorAddr, 8, 16, QChar('0'));
                ok = false;
                break;
            }
            qCDebug(log_chip_flash) << "MS2130S erased sector" << (i+1) << "/" << numSectors
    ...
    qCInfo(log_chip_flash) << "MS2130S write mode" << connectMode << "max burst:" << maxBurst;
    ...
    while (written < totalSize && ok) {
        ...
        if (!flashBurstWrite(chunkAddr, data.mid(written, static_cast<int>(toWrite)))) {
            qCWarning(log_chip_flash) << "MS2130S burst write failed at" << QString("0x%1").arg(chunkAddr, 8, 16, QChar('0'));
            ok = false;
        } else {
            written += toWrite;
        }
    ...
    if (ok) {
        qCInfo(log_chip_flash) << "MS2130S firmware write COMPLETED SUCCESSFULLY:" << totalSize << "bytes";
    } else {
        qCWarning(log_chip_flash) << "MS2130S firmware write FAILED after" << written << "/" << totalSize << "bytes";
    }
    ...
    if (ok) {
        ...
        if (rbOk && readback.size() >= 16) {
            ...
            qCInfo(log_chip_flash)  << "MS2130S flash readback  (first 16 B):" << toHex(readback,  16);
            qCInfo(log_chip_flash)  << "MS2130S firmware expected (first 16 B):" << toHex(expected, 16);
            if (match) {
                qCInfo(log_chip_flash) << "MS2130S flash verify PASS – write data confirmed in SPI flash";
                verifyPassed = true;
            } else {
                qCWarning(log_chip_flash) << "MS2130S flash verify FAIL – readback DOES NOT MATCH firmware!"
                                       " Possible causes: firmware file wrong format, write protocol bug.";
            }
        } else {
            qCWarning(log_chip_flash) << "MS2130S flash readback: 0xE7 read returned no data"
    ...
    if (ok && verifyPassed) {
        ...
        auto write16reg = [&](quint16 addr, quint8 val) -> bool {
            ...
            if (!r) qCWarning(log_chip_flash) << "MS2130S write16reg FAILED addr="
        ...
        auto clearResetFlag = [&]() {
            ...
            if (!s || !g)
                qCWarning(log_chip_flash) << "MS2130S clearResetFlag FAILED send=" << s << "get=" << g;
            else
                qCInfo(log_chip_flash) << "MS2130S clearResetFlag OK  0xFFFE="
                    << QString("0x%1").arg(recvBuf[flagReadOffset], 2, 16, QChar('0'));
        };

        // Stage 1: Run from ROM (enter bootloader ROM)
        qCInfo(log_chip_flash) << "MS2130S stage-1 soft reset (Run from ROM, mode=" << connectMode << ")...";
        clearResetFlag();
        ...
        qCInfo(log_chip_flash) << "MS2130S stage-1 reset sent; waiting for reconnect...";
        m_transport->close();
        ...
        if (reconnected) {
            ...
            qCInfo(log_chip_flash) << "MS2130S stage-2 soft reset (Run from RAM)...";
            clearResetFlag();
            write16reg(0xFF12, 0x00);  // Disable MCU write PRAM from ROM
            write16reg(0xFF11, 0x01);  // MCU run from RAM
            write16reg(0xFF13, 0x00);  // ys clk source select: 0=osc
            write16reg(0xFFFF, 0x01);  // soft reset
            qCInfo(log_chip_flash) << "MS2130S stage-2 reset sent. New firmware should now be active.";
        } else {
            qCWarning(log_chip_flash) << "MS2130S stage-1 reconnect timed out. Power cycle the device.";
        }
    } else if (ok) {
    ...
    if (ok) {
        qCInfo(log_chip_flash) << "MS2130S firmware update complete – device is re-enumerating";
    }
    return ok;
}
